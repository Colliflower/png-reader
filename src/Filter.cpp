...
namespace trv
{
static std::uint8_t paethPredictor(uint8_t left,
                                   std::uint8_t top,
                                   std::uint8_t topleft)
{
                 ...
                 std::size_t bpp)
{
	for (size_t scanline = 0; scanline < scanlines; ++scanline)
	{
		FilterMethod filterType =
		    static_cast<FilterMethod>(input[scanline * byteWidth + offset]);
		...
		for (size_t byte = 1; byte < byteWidth; ++byte)
		{
			...
			switch (static_cast<FilterMethod>(filterType))
			{
				case FilterMethod::Sub:
					{
						...
						std::uint8_t left =
						    input[scanline * byteWidth + byte - bpp + offset];
						value = left;
						break;
					}
				case FilterMethod::Up:
					{
						std::uint8_t top =
						    input[(scanline - 1) * byteWidth + byte + offset];
						value = top;
						break;
					}
				case FilterMethod::Average:
					{
						...
						value = static_cast<uint8_t>(
						    static_cast<uint16_t>(top + left) >> 1);
						break;
					}
				case FilterMethod::Paeth:
					{
						...
						if (scanline != 0 && byte > bpp)
							topleft = input[(scanline - 1) * byteWidth + byte -
							                bpp + offset];
